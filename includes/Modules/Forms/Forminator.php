 ...
 */
class Forminator extends FormStyler {
	...
	public function init(): void {
		 ...
		 * @since 3.2.0
		 *
		 * @param Forminator $module The current module instance.
		 */
		...
		$this->icon_path        = HelperUtil::fix_slash( divi_squad()->get_icon_path() . '/forminator.svg' );
		...
		$fields = array(
			'form_id'               => Utils::add_select_box_field(
				...
				array(
					...
					'options'          => FormsUtil::get_forms_by( 'forminator' ),
	...
	protected function squad_get_additional_design_fields(): array {
		...
		$fields['field_width'] = Utils::add_range_field(
	...
	protected function squad_get_font_fields(): array {
		 ...
		 * @param Forminator $module      Current module instance.
		 */
	...
	public static function squad_form_styler__get_form_html( array $attrs ): string {
		...
		if ( '' === $attrs['form_id'] || FormsUtil::DEFAULT_FORM_ID === $attrs['form_id'] || ! class_exists( 'Forminator_API' ) ) {
			return '';
		}
		...
		if ( '' === $form_id_raw ) {
			$collection = FormsUtil::get_forms_by( 'forminator', 'id' );
