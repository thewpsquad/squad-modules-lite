...
class Testimonial extends Module {
	...
	public static function render_callback( array $attrs, string $child_modules_content, WP_Block $block, $elements ): string {
		try {
			...
			$inner = $attrs['testimonials']['innerContent']['desktop']['value'] ?? array();
			$uid   = self::get_instance_uid( $block );

			$inline_css = self::get_grid_css( $inner, $uid );

			$grid_html = sprintf(
				'%1$s<div class="squad-testimonials squad-testimonials--grid %2$s">%3$s</div>',
	...
	protected static function get_instance_uid( WP_Block $block ): string {
		...
		return ( null !== $uid && '' !== $uid )
			? 'squad-ts-' . $uid
			: 'squad-ts-' . substr( md5( $raw ), 0, 10 );
	}

	/**
	 * Build the responsive grid CSS for the testimonials wrapper.
	 *
	...
	protected static function get_grid_css( array $inner, string $uid ): string {
		...
		$css  = ".{$uid}.squad-testimonials--grid{display:grid;grid-template-columns:repeat({$columns},minmax(0,1fr));gap:{$row_value} {$col_value};}";
		$css .= "@media(max-width:980px){.{$uid}.squad-testimonials--grid{grid-template-columns:repeat(2,minmax(0,1fr));}}";
		$css .= "@media(max-width:767px){.{$uid}.squad-testimonials--grid{grid-template-columns:1fr;}}";
