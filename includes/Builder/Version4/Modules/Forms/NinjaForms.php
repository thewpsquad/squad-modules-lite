 ...
 * @package DiviSquad
 */
class NinjaForms extends Form_Styler {
		...
		return array(
			'form_id'               => divi_squad()->d4_module_helper->add_select_box_field(
				...
				array(
					...
					'options'          => divi_squad()->form_element->get_forms_by( 'ninja_forms' ),
	...
	public static function squad_form_styler__get_form_html( array $attrs ): string {
		...
		if ( ! function_exists( '\Ninja_Forms' ) || '' === $attrs['form_id'] || divi_squad()->form_element::DEFAULT_FORM_ID === $attrs['form_id'] ) {
			return '';
		}
		...
		if ( '' === $form_id_raw ) {
			$collection = divi_squad()->form_element->get_forms_by( 'ninja_forms', 'id' );
