...
/**
 * SureForms Form Styler Module Class
 *
 * This file contains the SureForms class which extends the FormStyler
 * to provide custom styling options for SureForms within the Divi Builder.
 *
 * @since   1.4.7
 ...
 * for SureForms within the Divi Builder interface.
 *
 * @since   1.4.7
 * @package DiviSquad
 */
class Sure_Forms extends Form_Styler {
	...
	public function init(): void {
		/**
		 * Fires before the SureForms module is initialized.
		 *
		 * @since 3.2.0
		 *
		 * @param self $module The current module instance.
		 */
		do_action( 'divi_squad_before_module_sureforms_init', $this );

		$this->name      = esc_html__( 'SureForms', 'squad-modules-for-divi' );
		$this->plural    = esc_html__( 'SureForms', 'squad-modules-for-divi' );
		$this->icon_path = divi_squad()->get_icon_path( 'sureforms.svg' );

		$this->slug             = 'disq_form_styler_sureforms';
		$this->vb_support       = 'on';
		 ...
		 * @param self   $module    The current module instance.
		 */
		$this->icon_path = apply_filters( 'divi_squad_module_sureforms_icon_path', $this->icon_path, $this );
		...
		/**
		 * Filters the CSS selectors for the SureForms module.
		 *
		 * @since 3.2.0
		 *
		 ...
		 * @param self                                 $module    The current module instance.
		 */
		$this->squad_css_selectors = apply_filters( 'divi_squad_form_styler_module_sureforms_css_selectors', $selectors, $this );

		$this->squad_init_selectors();

		/**
		 * Fires after the SureForms module has been initialized.
		 *
		 * @since 3.2.0
		 *
		 * @param self $module The current module instance.
		 */
		do_action( 'divi_squad_after_module_sureforms_init', $this );
	}
	...
	public function get_settings_modal_toggles(): array {
		...
		// Add SureForms-specific toggle sections.
		$sureforms_toggles = array(
		...
		/**
		 * Filters the SureForms-specific toggle sections.
		 *
		 * @since 3.2.0
		 *
		 * @param array $toggles           The base toggles
		 * @param array $sureforms_toggles The SureForms-specific toggles
		 */
		$sureforms_toggles = apply_filters( 'divi_squad_module_sureforms_toggles', $sureforms_toggles, $toggles );
	...
	public function get_advanced_fields_config(): array {
		...
		/**
		 * Filters the advanced fields configuration for the SureForms module.
		 *
		 * @since 3.2.0
		 *
		 ...
		 * @param self  $module          The current module instance.
		 */
		return apply_filters( 'divi_squad_module_sureforms_advanced_fields', $advanced_fields, $this );
	}
	...
	public function render( $attrs, $content, $render_slug ): string {
		try {
			/**
			 * Fires before the SureForms module is rendered.
			 *
			 * @since 3.2.0
			 *
			 ...
			 * @param self  $module The current module instance.
			 */
			do_action( 'divi_squad_before_module_sureforms_render', $attrs, $this );

			// Check if SureForms is installed.
			if ( ! defined( 'SRFM_VER' ) ) {
				$message = esc_html__( 'SureForms plugin is not installed or activated. Please install and activate the plugin to use this module.', 'squad-modules-for-divi' );

				/**
				 * Filters the message shown when SureForms is not installed.
				 *
				 * @since 3.2.0
				 *
				 ...
				 * @param array  $attrs   The module attributes.
				 */
				$message = apply_filters( 'divi_squad_module_sureforms_not_activated_message', $message, $attrs );
			...
			if ( '' === $form_html ) {
				...
				if ( DiviUtil::is_fb_enabled() ) {
					 ...
					 * @param array  $attrs   The module attributes.
					 */
					$message = apply_filters( 'divi_squad_module_sureforms_no_form_message_vb', $message, $attrs );
				...
				if ( is_user_logged_in() ) {
					$message = esc_html__( 'No form selected from the SureForms Settings.', 'squad-modules-for-divi' );
					 ...
					 * @param array  $attrs   The module attributes.
					 */
					$message = apply_filters( 'divi_squad_module_sureforms_no_form_message_frontend', $message, $attrs );
				 ...
				 * @param self   $module    The current module instance.
				 */
				return apply_filters( 'divi_squad_module_sureforms_empty_form_html', '', $attrs, $this );
			}
			...
			/**
			 * Filters the final HTML output of the SureForms module.
			 *
			 * @since 3.2.0
			 *
			 ...
			 * @param self   $module    The current module instance.
			 */
			return apply_filters( 'divi_squad_module_sureforms_form_html', $form_html, $attrs, $this );
		} catch ( Throwable $e ) {
			$this->log_error(
				$e,
				array(
					...
					'module'      => 'SureForms',
					'method'      => 'render',
					'message'     => 'Error occurred while rendering SureForms module',
				)
			);
	...
	/**
	 * Get CSS selectors for the SureForms module.
	 *
	 * @since  1.4.7
	 * @access protected
	 *
	...
	protected function squad_get_css_selectors(): array {
		$css_selectors = array(
			'form'          => array(
				'wrapper' => "$this->main_css_element div .srfm-form-container",
				'form'    => "$this->main_css_element div .srfm-form-container form",
			),
			'fields'        => array(
				'all'      => $this->get_field_selector(),
				'input'    => "$this->main_css_element div .srfm-form-container form input",
				'textarea' => "$this->main_css_element div .srfm-form-container form textarea",
				'select'   => "$this->main_css_element div .srfm-form-container form select",
			),
			'typography'    => array(
				'labels'       => "$this->main_css_element div .srfm-form-container form .srfm-block-label",
				'placeholders' => array(
					'normal' => "$this->main_css_element div .srfm-form-container form input::placeholder, $this->main_css_element div .srfm-form-container form textarea::placeholder",
					'hover'  => "$this->main_css_element div .srfm-form-container form input:hover::placeholder, $this->main_css_element div .srfm-form-container form textarea:hover::placeholder",
				),
			),
			'submit_button' => array(
				'all' => "$this->main_css_element div .srfm-form-container form .srfm-submit-btn",
			),
			'messages'      => array(
				'validation' => "$this->main_css_element div .srfm-form-container .srfm-error-message",
				'error'      => "$this->main_css_element div .srfm-form-container .srfm-error-message",
				'success'    => "$this->main_css_element div .srfm-form-container .srfm-success-box",
			),
		);
		 ...
		 * @param self  $module        The current module instance.
		 */
		return apply_filters( 'divi_squad_module_sureforms_css_selectors', $css_selectors, $this );
	}
	...
	protected function squad_init_selectors(): void {
		...
		/**
		 * Fires after the selectors have been initialized for the SureForms module.
		 *
		 * @since 3.2.0
		 *
		 * @param self $module The current module instance.
		 */
		do_action( 'divi_squad_module_sureforms_after_init_selectors', $this );
	}
	...
	protected function squad_get_module_stylesheet_selectors( array $attrs ): array {
		 ...
		 */
		do_action( 'divi_squad_module_sureforms_before_get_stylesheet_selectors', $attrs, $this );

		$options = parent::squad_get_module_stylesheet_selectors( $attrs );

		// Add SureForms-specific style options.
		$forms_options = array(
		...
		/**
		 * Filters the SureForms-specific style options.
		 *
		 * @since 3.2.0
		 *
		 * @param array $forms_options The SureForms-specific options
		 * @param array $options       The base options
		 * @param array $attrs         Module attributes
		 */
		$forms_options = apply_filters( 'divi_squad_module_sureforms_style_options', $forms_options, $options, $attrs );
		 ...
		 * @param self  $module  Current module instance
		 */
		return apply_filters( 'divi_squad_module_sureforms_stylesheet_selectors', $options, $attrs, $this );
	}
	...
	protected function squad_get_general_fields(): array {
		$fields = array(
			'form_id'               => divi_squad()->d4_module_helper->add_select_box_field(
				...
				array(
					'description'      => esc_html__( 'Choose the SureForms form to display.', 'squad-modules-for-divi' ),
					'options'          => divi_squad()->forms_element->get_forms_by( 'sureforms' ),
		...
		/**
		 * Filters the general fields for the SureForms module.
		 *
		 * @since 3.2.0
		 *
		 ...
		 * @param self  $module The current module instance.
		 */
		return apply_filters( 'divi_squad_module_sureforms_general_fields', $fields, $this );
	}
	...
	protected function get_field_selector(): string {
		...
		$selectors = array_map(
			fn( $field ) => "$this->main_css_element div .srfm-form-container form $field",
			$allowed_fields
		);

		/**
		 * Filters the field selectors for the SureForms module.
		 *
		 * @since 3.2.0
		 *
		 ...
		 * @param self  $module    The current module instance.
		 */
		$selectors = apply_filters( 'divi_squad_module_sureforms_field_selectors', $selectors, $this );
	...
	protected function squad_get_font_fields(): array {
		 ...
		 * @param self  $module      Current module instance.
		 */
		return apply_filters( 'divi_squad_module_sureforms_font_fields', $font_fields, $this );
	}
	...
	protected function squad_get_border_fields(): array {
		 ...
		 * @param self  $module        Current module instance.
		 */
		return apply_filters( 'divi_squad_module_sureforms_border_fields', $border_fields, $this );
	}
	...
	protected function squad_get_box_shadow_fields(): array {
		 ...
		 * @param self  $module            Current module instance.
		 */
		return apply_filters( 'divi_squad_module_sureforms_box_shadow_fields', $box_shadow_fields, $this );
	}
	...
	public static function squad_form_styler__get_form_html( array $attrs ): string {
		 ...
		 */
		$attrs = (array) apply_filters( 'divi_squad_module_sureforms_get_form_html_attrs', $attrs );

		if ( ! defined( 'SRFM_VER' ) || '' === $attrs['form_id'] || divi_squad()->forms_element::DEFAULT_FORM_ID === $attrs['form_id'] ) {
			return '';
		}
		...
		if ( '' === $form_id_raw ) {
			$collection = divi_squad()->forms_element->get_forms_by( 'sureforms', 'id' );
			 ...
			 * @param array $attrs      The form attributes.
			 */
			$collection = (array) apply_filters( 'divi_squad_module_sureforms_forms_collection', $collection, $attrs );
		...
		$shortcode = sprintf( '[sureforms id="%s"]', esc_attr( $form_id_raw ) );

		/**
		 * Filters the SureForms shortcode before processing .
		 *
		 * @since 3.2.0
		 *
		 * @param string $shortcode The form shortcode. Default is [sureforms id="form_id"].
		 * @param string $form_id   The form ID.
		 * @param array  $attrs     The form attributes .
		 */
		$shortcode = apply_filters( 'divi_squad_module_sureforms_form_shortcode', $shortcode, $form_id_raw, $attrs );
		 ...
		 * @param array  $attrs     The form attributes.
		 */
		return apply_filters( 'divi_squad_module_sureforms_processed_form_html', $html, $form_id_raw, $shortcode, $attrs );
	}
}
