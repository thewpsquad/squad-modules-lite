...
class Pricing_Table extends Module {
	...
	public function init(): void {
		...
		$this->icon_path = divi_squad()->get_icon_path( 'pricing-table.svg' );

		$this->slug             = 'disq_pricing_table';
		$this->child_slug       = 'disq_pricing_table_item';
		$this->vb_support       = 'on';
	...
	public function apply_grid_css( string $render_slug ): void {
		...
		$col_gap   = self::sanitize_css_length( (string) $this->prop( 'column_gap', '24px' ) );
		$row_gap   = self::sanitize_css_length( (string) $this->prop( 'row_gap', '24px' ) );
